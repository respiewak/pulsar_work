...
def proc_args():
    ...
    pars.add_argument("psrs", nargs="*", required=False)
    pars.add_argument("pars", nargs="*", required=False)
    pars.add_argument("-r", "--root_dir", default="../")
    pars.add_argument("-d", "--db_file", default="test.db")
    return(vars(pars.parse_args()))


def main(psrs=None, pars=None, root_dir='../', db_file='test.db'):
    if psrs is None and pars is None:
        raise(RuntimeError("Not enough information provided"))
    ...
    if not os.path.exists(db_file):
        ...
        if not os.path.exists(db_file):
            raise(RuntimeError("Invalid path to db_file"))

    if isinstance(pars, Iterable) \
       and not os.path.exists(os.path.join(root_dir+pars[0])):
        if not os.path.exists(root_dir+fold_eph_dir):
            raise RuntimeError("Invalid path to par file directory")
    ...
    for psr in psrs:
        ...
        with open(temp_log, 'a') as f:
            if rms_out is None:
                print("Could not read TRES from output par file: {}"
                      .format(psr))
                f.write(psr+"\t#FAILED\n")
                #raise RuntimeError("Could not read TRES from output par file")
            else:
                if rms_out/1e3 > 2*psr_p0:
                    print("WARNING: RMS greater than period for {}"
                          .format(psr))
                    f.write(psr+"\t#FAILED\n")
                else:
                    f.write(psr+"\t#Passed\n")
